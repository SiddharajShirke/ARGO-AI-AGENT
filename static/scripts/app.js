...
class ArgoApp {
    ...
    async initializeCore() {
        ...
        if (window.appState && typeof appState.init === 'function') {
            ...
            console.warn('⚠️ AppState not available, using fallback');
            window.appState = this.createFallbackState();
        }

        // Initialize API client
        if (window.api && typeof api.init === 'function') {
            api.init();
        } else {
            console.warn('⚠️ API client not available, using fallback');
            window.api = this.createFallbackAPI();
        }

        // Initialize utilities
        if (window.utils && typeof utils.init === 'function') {
            utils.init();
        } else {
            console.warn('⚠️ Utils not available, using fallback');
            window.utils = this.createFallbackUtils();
        }

        // Initialize DOM helpers
        if (window.domHelpers && typeof domHelpers.init === 'function') {
            domHelpers.init();
        } else {
            console.warn('⚠️ DOM helpers not available, using fallback');
            window.domHelpers = this.createFallbackDOMHelpers();
        }
    }
    ...
    createFallbackState() {
        return {
            state: new Map(),
            init() { console.log('📊 Fallback state initialized'); },
            get(key) { return this.state.get(key); },
            set(key, value) { this.state.set(key, value); },
            getAll() { return Object.fromEntries(this.state); },
    ...
    createFallbackAPI() {
        return {
            ...
            init() { console.log('🌐 Fallback API initialized'); },
    ...
    createFallbackUtils() {
        return {
            init() { console.log('🛠️ Fallback utils initialized'); },
            formatTime(date) {
    ...
    createFallbackDOMHelpers() {
        return {
            init() { console.log('🏗️ Fallback DOM helpers initialized'); }
        };
    }
}
